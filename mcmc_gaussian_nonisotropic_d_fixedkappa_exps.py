...
def main_simu(d, nb_exps=100, nb_iters=40000, seed=1):
    ...
    error_hmc_all[:nb_hmc_iters,:], x_hmc, ac_hmc, _ = mcmc.hmc(init_distr, grad_f_local, f_local, error_quantile,
                                               stepchoice=3, kappa=kappa, L=L, L3=L, cK=cK, nb_iters=nb_hmc_iters, 
                                               nb_exps=nb_exps)
    ...
    np.save('%s/warm_gaussian_nonisotropic_d%d_kappa23_cK%d_iters%d_exps%d_seed%d.npy' %(save_path, d, int(cK), nb_iters, nb_exps, seed), result)

if __name__ == '__main__':
    d = int(sys.argv[1])
    seed = int(sys.argv[2])
    nb_exps = int(sys.argv[3])
    main_simu(d=d, nb_exps=nb_exps, nb_iters=500000, seed=seed)
